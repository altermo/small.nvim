...
function M.send_png_packet(payload,last,first,width,height,lcmd)
    M.write('\x1b_G')
    local cmd={'m='..(last and '0' or '1'),lcmd}
    if first then
...
function M.render_direct(source,x,y,width,height,win)
...
function M.render_file(source,x,y,width,height,win)
    if not M.is_png(source) then error('source is not a png') end
    if vim.fn.filereadable(source)==0 then error('source is not a file') end
    if win then
        local row,col=unpack(vim.api.nvim_win_get_position(win))
        y,x=y+row,x+col
    end
    local data=vim.base64.encode(source)
    local chunks=M.chunckify(data)
    for k,chunk in ipairs(chunks) do
        if x and y then M.write('\x1b['..y..';'..x..'H') end
        M.send_png_packet(chunk,k==#chunks,k==1,width,height,'t=f')
    end
end
function M.render(...)
    M.render_file(...)

    ...
    for k,v in vim.spairs(M.notifs) do
        if v.dispand then
            if vim.api.nvim_get_current_buf()==v.buf then
                ...
                if v.buf then vim.api.nvim_buf_delete(v.buf,{force=true}) end
                M.notifs[k]=nil
            end
            goto continue
        end
        ...
        if not v.win then
            ...
            v.win=vim.api.nvim_open_win(v.buf,false,vim.tbl_extend('force',{style='minimal',noautocmd=true},winopt))

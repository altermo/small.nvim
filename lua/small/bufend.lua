...
function M.lock_file(key) require'small.lib.select'(M.get_list(),{},function (i) M.locked_files[key]=i end) end
function M.goto_file(key)
    ...
    elseif #dict==0 then
        ...
        require'small.lib.select'(vim.fs.find(function (name,path) return name:sub(1,1)==key and not path:sub(#vim.fn.getcwd()):match('/%.') end,{type='file',limit=math.huge}),{},function (file) vim.cmd.edit(file) end)
